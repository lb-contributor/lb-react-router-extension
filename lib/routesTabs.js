  ...
  _createClass(RoutesTabs, [{
    ...
    value: function addTab(content, location) {
      ...
      if (panes.length === 0 && path !== firstpath) {
        ...
        _react2.default.Children.forEach(this.props.children, function (element) {
          ...
          if (match == null) {
            child = element;
            match = path ? (0, _matchPath2.default)(firstpath, {
              path: path,
              exact: exact,
              strict: strict,
              sensitive: sensitive
            }) : _this4.context.router.route.match;
          }
        });
        var element = match ? _react2.default.cloneElement(child, { location: location, computedMatch: match }) : null;
        panes.push({
    ...
    value: function remove(targetKey) {
      ...
      panes.some(function (pane) {
        return pane.key === targetKey && delete _this5.activeCallbackFuncs[pane.path];
      });
      if ($panes.length > 0) {
        var activeKey = this.state.active.previous.key;
        var hasHistory = $panes.some(function (pane) {
    ...
    value: function render() {
      ...
      return _react2.default.createElement(
        ...
        this.state.panes.map(function (pane) {
          return _react2.default.createElement(
            TabPane,
            { tab: pane.title, key: pane.key, closable: pane.key !== 'tab$0' },
            pane.content
          );
        })
      );
